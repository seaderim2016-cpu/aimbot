...
typedef enum _FILE_INFORMATION_CLASS {
  ...
  FileRenameInformationExBypassAccessCheck, // (kernel-mode only);
                                            // FILE_RENAME_INFORMATION_EX
  FileDesiredStorageClassInformation, // FILE_DESIRED_STORAGE_CLASS_INFORMATION
                                      // // since REDSTONE2
  FileStatInformation,                // FILE_STAT_INFORMATION
  FileMemoryPartitionInformation, // FILE_MEMORY_PARTITION_INFORMATION // since
                                  // REDSTONE3
  ...
  FileLinkInformationExBypassAccessCheck, // (kernel-mode only);
...
#define FILE_RENAME_SUPPRESS_STORAGE_RESERVE_INHERITANCE                       \
  0x00000008 // since REDSTONE5
...
typedef enum _IO_PRIORITY_HINT {
  IoPriorityVeryLow =
      0,              // Defragging, content indexing and other background I/Os.
...
typedef struct _FILE_REMOTE_PROTOCOL_INFORMATION {
  // Structure Version
  USHORT StructureVersion; // 1 for Win7, 2 for Win8 SMB3, 3 for Blue SMB3, 4
                           // for RS5
...
// private
#ifndef _FILE_STAT_INFORMATION_
...
// private
#ifndef _FILE_STAT_LX_INFORMATION_
...
// private
#ifndef _FILE_CASE_SENSITIVE_INFORMATION_
...
typedef enum _FSINFOCLASS {
  ...
  FileFsMetadataSizeInformation, // FILE_FS_METADATA_SIZE_INFORMATION // since
                                 // THRESHOLD
  FileFsFullSizeInformationEx,   // FILE_FS_FULL_SIZE_INFORMATION_EX // since
                                 // REDSTONE5
...
NTSTATUS
NTAPI
NtCreateNamedPipeFile(_Out_ PHANDLE FileHandle, _In_ ULONG DesiredAccess,
                      _In_ POBJECT_ATTRIBUTES ObjectAttributes,
                      _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                      _In_ ULONG ShareAccess, _In_ ULONG CreateDisposition,
...
NTSTATUS
NTAPI
NtQueryInformationByName(_In_ POBJECT_ATTRIBUTES ObjectAttributes,
                         _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                         _Out_writes_bytes_(Length) PVOID FileInformation,
                         _In_ ULONG Length,
                         _In_ FILE_INFORMATION_CLASS FileInformationClass);
...
NTSTATUS
NTAPI
NtQueryDirectoryFileEx(_In_ HANDLE FileHandle, _In_opt_ HANDLE Event,
                       _In_opt_ PIO_APC_ROUTINE ApcRoutine,
                       _In_opt_ PVOID ApcContext,
                       _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                       _Out_writes_bytes_(Length) PVOID FileInformation,
...
NtQueryQuotaInformationFile(_In_ HANDLE FileHandle,
                            ...
                            _In_ ULONG Length, _In_ BOOLEAN ReturnSingleEntry,
                            _In_reads_bytes_opt_(SidListLength) PVOID SidList,
                            _In_ ULONG SidListLength, _In_opt_ PSID StartSid,
                            _In_ BOOLEAN RestartScan);
...
NTSTATUS
NTAPI
NtNotifyChangeDirectoryFileEx(_In_ HANDLE FileHandle, _In_opt_ HANDLE Event,
                              _In_opt_ PIO_APC_ROUTINE ApcRoutine,
                              _In_opt_ PVOID ApcContext,
                              _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                              _Out_writes_bytes_(Length) PVOID Buffer,
                              _In_ ULONG Length, _In_ ULONG CompletionFilter,
                              _In_ BOOLEAN WatchTree,
                              _In_opt_ DIRECTORY_NOTIFY_INFORMATION_CLASS
...
NtQueryIoCompletion(
    ...
    _Out_writes_bytes_(IoCompletionInformationLength)
...
NTSTATUS
NTAPI
NtCreateWaitCompletionPacket(_Out_ PHANDLE WaitCompletionPacketHandle,
                             _In_ ACCESS_MASK DesiredAccess,
                             _In_opt_ POBJECT_ATTRIBUTES ObjectAttributes);
...
NTSTATUS
NTAPI
NtCancelWaitCompletionPacket(_In_ HANDLE WaitCompletionPacketHandle,
...
NtNotifyChangeSession(
    _In_ HANDLE SessionHandle, _In_ ULONG ChangeSequenceNumber,
    _In_ PLARGE_INTEGER ChangeTimeStamp, _In_ IO_SESSION_EVENT Event,
    _In_ IO_SESSION_STATE NewState, _In_ IO_SESSION_STATE PreviousState,
...
// Reparse structure for FSCTL_SET_REPARSE_POINT, FSCTL_GET_REPARSE_POINT,
...
#define FSCTL_PIPE_DISABLE_IMPERSONATE                                         \
  CTL_CODE(FILE_DEVICE_NAMED_PIPE, 17, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define FSCTL_PIPE_SILO_ARRIVAL                                                \
  CTL_CODE(FILE_DEVICE_NAMED_PIPE, 18, METHOD_BUFFERED, FILE_WRITE_DATA)
#define FSCTL_PIPE_CREATE_SYMLINK                                              \
  CTL_CODE(FILE_DEVICE_NAMED_PIPE, 19, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define FSCTL_PIPE_DELETE_SYMLINK                                              \
  CTL_CODE(FILE_DEVICE_NAMED_PIPE, 20, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define FSCTL_PIPE_QUERY_CLIENT_PROCESS_V2                                     \
  CTL_CODE(FILE_DEVICE_NAMED_PIPE, 21, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
// Input for FSCTL_PIPE_SET_CLIENT_PROCESS, Output for
...
typedef struct _FILE_PIPE_CLIENT_PROCESS_BUFFER_EX {
  ...
  WCHAR ClientComputerBuffer[FILE_PIPE_COMPUTER_NAME_LENGTH +
                             1]; // null-terminated
...
// resolved as \Device\NamedPipe\target instead of
// \Device\Silos\37\Device\NamedPipe\target
//
#define FILE_PIPE_SYMLINK_FLAG_GLOBAL 0x1

//
// A relative symlink will cause resolution of the symlink's target to occur
// relative to the root of the named pipe file system.  For example, if there is
// a symlink at
// \Device\NamedPipe\symlink that has a target called "target", then the target
// will be resolved as \Device\NamedPipe\target
//
...
#define MOUNTMGR_IS_VOLUME_NAME(s)                                             \
  (((s)->Length == 96 || ((s)->Length == 98 && (s)->Buffer[48] == '\\')) &&    \
   ...
   (s)->Buffer[19] == '-' && (s)->Buffer[24] == '-' &&                         \
   (s)->Buffer[29] == '-' && (s)->Buffer[34] == '-' && (s)->Buffer[47] == '}')
